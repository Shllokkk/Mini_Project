...
public class IncomePageController {
    ...
    private void saveDetails(ActionEvent event) throws Exception{
        ...
        String datainsert="insert into userincome values ('"+foreignkey+"','"+typefield.getText()+"',"+Integer.parseInt(amtfield.getText())+",'"+descfield.getText()+"',curdate())";

        Statement statement=connectdb.createStatement();
        int x=statement.executeUpdate(datainsert);
        if(x!=0) 
            System.out.println("income data inserted!");
        else 
            System.out.println("failed to insert income data!");
    }
}